...
### "Editorial War Room" — Whiteboard visual language × Clay color × Cohere typography

## Overview

AI Builders ID keeps its **Digital Whiteboard / War Room** soul — kraft-board canvas, sticky notes with tape, hand-sketched wonky borders, dot-grid texture, rough-marker highlights — but is re-lit with **Clay's warm cream + saturated 6-color palette** and re-typeset with **Cohere's monumental, tight-tracking editorial hierarchy**.

The synthesis works because of one lucky coincidence: Clay's 6-color feature-card palette (pink, teal, lavender, peach, ochre + cream) maps almost 1:1 onto the site's existing 6-color sticky-note system. Recoloring the notes with Clay hues is not a stretch — it's the same pattern the two systems already share.

The bigger move is typographic. Cohere's voice is a giant, tight, negatively-tracked display face — the opposite personality of a marker scrawl. Rather than replace the handwritten identity outright, **Permanent Marker is demoted from "headline font" to "annotation font"** (section tags, sticky-note titles, badges, doodled circles/arrows), and headlines move to **Space Grotesk run at Cohere's scale and tracking** — which works cleanly because Space Grotesk is literally Cohere's own documented display-font fallback. The result: a monumental, editorial headline with a handwritten annotation scribbled next to it — command-center clarity with war-room texture.

**Key Characteristics:**
- Canvas shifts from tan-kraft to Clay's warm cream (`#fffaf0`) — still reads as "board," now brighter and more premium.
...
- Two card languages coexist by zone: **wonky sketch cards** (sticky notes, testimonials, doodled callouts) for community/personality sections, and **clean flat Clay-style cards** (feature cards, CTA bands, pricing) for product/trust sections.
- Headlines run monumental and tight (Cohere scale) in Space Grotesk; Permanent Marker is reserved for annotations, tags, and sticky-note handwriting only.
- Section rhythm opens up from 72px to Clay/Cohere's 96px — more premium breathing room without losing density where it earns its keep (feature grids, research-style lists).
- Cream/light footer — already shared by all three source systems, no change needed.
- Dot-grid + sketch-border + tape + rough-underline highlighter — fully preserved as the tactile, hand-crafted signature.

## Colors

### Brand & Accent (from Clay)
...
- **Brand Teal** (`{colors.brand-teal}` — `#1a3a3a`): Reserved for the "featured / pinned" signal — dark sticky note, featured pricing card. Not used as a pastel tint (too dark) — used at full saturation, sparingly, the way Clay uses it for its featured pricing tier.
...
### Sticky-Note Pastels (Clay hues, tinted to note-lightness ~86–90% L)
Each note color is the *same hue* as a Clay brand color, lightened to sticky-note range. This keeps the note system and the feature-card system visually related, not arbitrary.
...
Cycle notes/feature-cards through this list without repeating the same color twice in a row (same rule Clay uses for its feature-card grid).
...
- **Hairline Flat** (`{colors.hairline}` — `#e5e5e5`): New, from Clay — reserved for the clean Clay-style components only (feature-card-flat, forms, product-mockup-card).
...
### Font Family
- **Display**: `'Space Grotesk'`, weight 600–700, run at Cohere's monumental scale/tracking. Fallback: `-apple-system, BlinkMacSystemFont, sans-serif`.
- **Body**: `'Space Grotesk'`, weight 400.
- **Annotation / Hand**: `'Permanent Marker', cursive` — annotation-only role (see Principles).
- **Mono / Eyebrow Label**: `'Space Grotesk', monospace` fallback, uppercase, letter-spaced — Cohere's mono-label pattern, reused for eyebrow tags above section headlines.
...
### Principles
- **Space Grotesk carries the hierarchy.** All headlines (h1–h4, section heads, card titles) run in Space Grotesk at large size, tight line-height, and negative tracking — this is where the "Cohere feel" lives.
...
- **Section padding:** bumped from 72px → `{spacing.section}` 96px (Clay/Cohere rhythm) for major editorial bands. Feature grids, research-style lists, and dense product sections may keep tighter internal spacing — the *between-section* rhythm is what opens up, not every internal grid.
- **Card padding:** 24–32px, unchanged.

### Grid & Container
- **Max content width:** 1200–1280px centered (bumped slightly from 1180px to align with Clay/Cohere).
...
Whitespace now does double duty: it's still the "board with room to pin things" feeling, but with Clay/Cohere's more generous section gaps it also reads as trustworthy/premium rather than purely playful — closer to "a well-organized war room" than "a cluttered one."
...
The shadow opacity is slightly softened to sit comfortably on Clay's brighter cream — the old values were tuned for the darker kraft-tan and read a touch heavy on `#fffaf0`.
...
**Clean (product/trust zone)** — new, from Clay, for feature-card-flat / cta-band / product-mockup-card / pricing:
- `{rounded.sm}` — 8px
...
### New (adapted from Clay)
- **`feature-card-flat`** — Clay-style saturated flat card, `{rounded.xl}` (24px), no shadow, cycles through the 6 brand hues. Holds an h3 in `{typography.title-md}`, a short body line, and a small product-UI fragment or icon. Text flips to `{colors.on-dark}` on the teal variant, stays `{colors.ink}` on the rest.
...
- Keep the footer cream/light — all three source systems already agree on this.
...
- Don't invent an 8th brand hue outside the Clay-derived set.
- Don't add heavy drop shadows to the new flat Clay-style cards — depth there comes from color contrast against cream, not shadow.
...
6. Keep dot-grid + tape + rough-underline on every page — they're non-negotiable brand signature, same status as Clay's cream canvas or Cohere's tight tracking.
...
| `h1, h2, h3, h4 { font-family: 'Permanent Marker' }` | `font-family: 'Space Grotesk'`, weight 600–700, Cohere-scale tracking. Permanent Marker moves to `.hand-tag` / `.hand-note` utility classes only. |
...
- Illustration style (any custom SVG/doodle assets beyond CSS-driven sticky notes and dot-grid) isn't covered — Clay's 3D claymation illustrations are explicitly **not** adopted here; this system stays flat/CSS-drawn, consistent with the existing site.
